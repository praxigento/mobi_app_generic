'use strict'
/**
 * Authentication data for test accounts.
 */
var result = {
    mage: {
        admin: {
            /* \Praxigento\App\Generic2\Cli\Cmd\Init\Sub\AclUser::USER_TESTER_ */
            tester: {
                user: 'tester',
                password: 'b4yHm6PNQ1PXeFsHzUDu'
            }
        },
        api: {
            /* \Praxigento\App\Generic2\Cli\Cmd\Init\Sub\AclUser::USER_ODOO_ */
            odoo: {
                user: 'odoo',
                password: '8sxUwQ5NsK2R5RUb'
            }
        },
        front: {
            /* \Praxigento\App\Generic2\Cli\Cmd\Init\Customers::$DEFAULT_PASSWORD_HASH */
            customer01: {
                email: 'customer_1@test.com',
                password: 'UserPassword12'
            },
            customer10: {
                email: 'customer_10@test.com',
                password: 'UserPassword12'
            },
            customerAnon: {
                email: 'mobi.anon@gmail.com',
                password: 'UserPassword12'
            },
            customerReferral: {
                email: 'mobi.referral@gmail.com',
                password: 'UserPassword12'
            }
        }
    },
    odoo: {
        api: {
            mage: {
                dbname: 'odoo_mobi_test',
                admin: 'magento',
                password: 'HekGpzkY89bt'
            }
        },
        shop: {},
        web: {
            consignee:{
                name: 'Consignee MOBI Casper',
                password: 'UserPassword12'
            }
        }
    },
    gmail: {
        customerAnon: {
            email: 'mobi.anon@gmail.com',
            password: 'F7HVVFxB4ZyhNDbt0yp6'
        },
        customerReferral: {
            email: 'mobi.referral@gmail.com',
            password: 'TGxduvOCg1XlgkNkkeyn'
        }
    }
}

module.exports = result;